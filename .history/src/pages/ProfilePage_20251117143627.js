...
const ProfilePage = () => {
  ...
  /**
   * Načtení uživatelských dat z backendu.
   * Pouze při načtení stránky.
   */
  useEffect(() => {
  ...
  /**
   * Výběr výchozí profilové fotky podle pohlaví.
   */
  ...
  /**
   * Vrací správnou URL profilové fotky:
   *  - defaultní avatar podle pohlaví
   *  - frontendová cesta (/avatars)
   *  - veřejná URL ze Supabase / backend relativní cesta (/uploads)
   */
   ...
   * U pole gender řeší také automatický výběr avataru,
   * pokud uživatel nemá vlastní nahranou fotku.
   */
  const handleChange = (e) => {
    ...
    if (name === "gender") {
      ...
      // refresh avatru (řeší problémy s cache)
  ...
  /**
   * Upload nové profilové fotky přes Supabase Storage.
   * Postup:
   *  1) upload souboru do bucketu
   *  2) získání veřejné URL
   *  3) uložení URL na backend do DB
  ...
  /**
   * Uložení změn profilu (PUT na backend).
   * Aktualizuje také jméno v localStorage pro zbytek aplikace.
   */

 ...
 * DashboardPage.js
 *
 * Stránka s přehledem výkonnostních statistik uživatele.
 *
 * Načítá sportovní události z backendu pro přihlášeného uživatele.
 * Umožňuje filtrování podle časového období (den / týden / měsíc / rok / vše).
 * Umožňuje filtrování podle sportu.
 *
 * Zobrazuje hlavní metriky (vzdálenost, doba trvání, počet aktivit).
 *
 * Obsahuje tři hlavní grafy pomocí knihovny Recharts:
 *  - LineChart: trend vzdálenosti v čase
 *  - BarChart: součet času podle sportu
 *  - PieChart: procentuální podíl jednotlivých sportů
 */
...
// mapování enumů z backendu -> názvy pro uživatele
const SPORT_LABEL = {
  RUNNING: "Běh",
  CYCLING: "Kolo",
  SWIMMING: "Plavání",
...
// pořadí sportů pro grafy a výpočty
const SPORT_ORDER = ["RUNNING", "CYCLING", "SWIMMING", "OTHER"];

// dostupné časové filtry
const PERIODS = [
  { key: "day", label: "Den" },
...
// bezpečný převod ISO stringu na Date
const toDate = (dt) => {
  try {
...
// výpočet trvání události v minutách
...
const DashboardPage = () => {
  ...
  /**
   * Načítání sportovních událostí z backendu.
   * Probíhá pouze při načtení stránky.
   */
  useEffect(() => {
      ...
      try {
        ...
        const normalized = (data || [])
          ...
          .map((e) => {
            ...
            // výpočet trvání, pokud backend neposílá duration
            const durationMin =
  ...
  /**
   * Výpočet časového období podle aktuálního filtru.
   */
  ...
  /**
   * Filtrování událostí podle vybraného období a sportu.
   */
  ...
  /**
   * Výpočet hlavních metrik – vzdálenost, čas, počet aktivit.
   */
  ...
  /**
   * Trend vzdálenosti v čase – slouží jako vstup pro LineChart.
   * Indexace podle dnů/týdnů/měsíců podle zvoleného období.
   */
  const trendData = useMemo(() => {
    if (filtered.length === 0) return [];

    // krátké období – vykreslení po dnech
    ...
    // období "vše" – agregace po měsících
    const map = new Map();
    filtered.forEach((e) => {
      if (!e._start) return;
    ...
    <>
      ...
      <div className="dashboard-container">
        <div className="dashboard-card">
          ...
          {/* stav načítání a chyb */}
          {loading && <div className="dash-loading">Načítám data…</div>}
          {error && !loading && <div className="dash-error">{error}</div>}

          {/* obsah se zobrazí pouze pokud není chyba a data jsou načtena */}
          {!loading && !error && (
            <>
              {/* ovládací prvky – výběr období a sportu */}
